...
public class EmployeeService {
    ...
    @Caching(
            evict = {
                    @CacheEvict(value = "emp", beforeInvocation = true, key = "#employee.id"),
                    @CacheEvict(value = "emp" /*keyGenerator = "myKeyGenerator"*/, key = "'getEmps'"),
                    @CacheEvict(value = "emp", key = "#employee.lastName"),
                    @CacheEvict(value = "emp", key = "#employee.email"),
            }
    )
    public void deleteEmp(Employee employee) {
        System.out.println("deleteEmp:" + employee.getId());
        employeeMapper.deleteEmpById(employee.getId());
//        int i = 10 / 0;
    }
    ...
    /**
     * 这里由于巨大的坑,如果你使用了redis的 json序列化器,
     * 这里面由于返回的类型问题,会导致数据不能正常的反序列化,
     *
     * @return
     */
    @Caching(
            cacheable = {
                    @Cacheable(value = "emp" /*keyGenerator = "myKeyGenerator"*/, key = "'getEmps'")
            }
    )
